...
COMPOSE_CMD=(docker compose --profile stage6 \
  ...
  -f "$REPO_ROOT/docker-compose.override.stage6.yml" \
  -f "$REPO_ROOT/docker-compose.override.stage6.monitoring.yml" \
  -f "$REPO_ROOT/monitoring/vault/docker-compose.override.stage6.vault.yml")
...
ITERATIONS="$TRAFFIC_ITERATIONS" "$REPO_ROOT/scripts/ops/monitoring/traffic-gen.sh"
sleep 5
...
if [[ $(jq -r '.sealed' <<<"$VAULT_HEALTH") != "false" ]]; then
  ...
  if [[ -x "$REPO_ROOT/scripts/ops/vault/vault-unseal.sh" ]]; then
    "$REPO_ROOT/scripts/ops/vault/vault-unseal.sh"
    sleep 2
    VAULT_HEALTH=$(curl -sk "https://localhost:${VAULT_PORT}/v1/sys/health")
    if [[ $(jq -r '.sealed' <<<"$VAULT_HEALTH") != "false" ]]; then
...
VAULT_METRICS_TOKEN_FILE="$REPO_ROOT/monitoring/prometheus/vault-metrics.token"
