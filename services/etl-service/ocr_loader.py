...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("ocr_loader.log"), logging.StreamHandler()],
)
...
class OCRLoader:
    ...
    def __init__(self, database_url: str, chroma_url: str | None = None):
    ...
    def extract_lot_id(self, filename: str) -> int | None:
                ...
                for file_info in zip_ref.filelist:
                    if file_info.filename.lower().endswith(".pdf"):
                        try:
                            ...
                            logger.warning(
                                f"Failed to process {file_info.filename} in ZIP: {e}"
                            )
    ...
    def store_in_postgres(
        self, lot_id: int, filename: str, file_type: str, content: str
    ):
    ...
    async def store_in_chroma(
        self, attachment_id: int, lot_id: int, filename: str, content: str
    ):
    ...
    async def process_directory(
        self, path: Path, enable_embedding: bool = False
    ) -> dict[str, int]:
...
async def main():
    ...
    parser.add_argument(
        "--path", required=True, help="Path to directory containing PDF/ZIP files"
    )
    ...
    try:
        ...
        async with OCRLoader(args.db, args.embed) as loader:
            stats = await loader.process_directory(
                path, enable_embedding=bool(args.embed)
            )
