...
## Спринт 1 (1–2 недели, до 24.09.2025) — «Фундамент ETL + база»

\[ \] Стабилизировать ETL (Lots, TrdBuy, Contract, Rnu, Subjects → PostgreSQL)
\[ \] Настроить ChromaDB для текстов (семантический поиск)
\[ \] Поднять базовый веб-дашборд (Streamlit/FastAPI UI, без авторизации) → показать лоты из БД
\[ \] Настроить n8n для ежедневного ETL и уведомлений (Telegram как демо)
🎯 Метрика: Веб-UI показывает 100k лотов с фильтрацией по ключевому слову

## Спринт 2 (2–3 недели, до 15.10.2025) — «AI-аналитика + поставщики»

\[x\] Интеграция OCR (PDF/ZIP → текст → ChromaDB)
\[x\] Добавить проверку поставщиков (RNU/Subjects) → флаги «⚠ риск»
\[ \] Реализовать AI-аналитику (Flowise → резюме лота, ROI, риск)
\[ \] Доработать веб-дашборд: вкладки «Аналитика», «Поставщики», «История конкурентов»
🎯 Метрика: 80% точность поиска «аренда автобуса» в PDF, веб-UI показывает флаги риска и ROI

## Спринт 3 (2 недели, до 30.10.2025) — «Пользовательский опыт + пилот»

\[ \] Веб-UI: персонализированные уведомления (фильтры ROI, риск, КАТО/ОКЭД)
\[ \] Добавить автогенерацию документов (заявки, жалобы)
\[ \] Реализовать Freemium-модель (лимиты + оплата за расширенные функции)
\[ \] Провести нагрузочные тесты (100k запросов, 99% uptime)
🎯 Метрика: 20 тестовых пользователей через веб-UI, 80% открываемость алертов
