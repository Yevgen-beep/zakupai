...
\[ \] CI/CD оптимизация
\[ \] matrix build в .github/workflows/ci.yml
\[ \] кеширование pip зависимостей
\[ \] кеширование apt пакетов (tesseract, jq)
\[ \] добавить Docker layer caching
DoD: время CI сократилось минимум на 30%

# ========================= Week 2 - Фичи ядра

\[ \] ETL Batch Upload
\[ \] добавить эндпоинт /etl/upload-batch для CSV/XLSX
\[ \] реализовать обработку (сначала синхронно, очередь позже)
\[ \] интегрировать с n8n workflow
\[ \] добавить логирование ошибок парсинга CSV/XLSX
DoD: CSV ≤10 MB обрабатывается \<5 сек, ошибок ≤1%

\[ \] RNU-валидация участников
\[ \] реализовать метод /validate_rnu в risk-engine
\[ \] добавить кеширование результатов (PostgreSQL/Redis, TTL=24ч)
\[ \] интегрировать команду /rnu \<БИН> в Telegram боте
\[ \] предусмотреть fallback: если кеш пуст → прямой запрос в API
DoD: ответ при кэше ≤500 мс, успешная валидация ≥95% случаев

\[ \] Расширенный поиск
\[ \] добавить фильтры по суммам (min_amount, max_amount)
\[ \] добавить фильтры по статусам лотов
\[ \] реализовать автодополнение в Web UI (autocomplete.js)
\[ \] протестировать автодополнение на реальных кириллических данных
